...
<xsl:template match="/castxml2pony/renderuses/renderuse[@render!='0']">
  ...
  <xsl:result-document href="uses/{$filename}.use" method="text">
...
<xsl:text>/*
  ...
  Original Name: <xsl:value-of select="$fnode/@name"/>
    <xsl:message>[<xsl:value-of select="$render"/>][<xsl:value-of select="$fnode/@name"/>]<xsl:value-of select="/castxml2pony/CastXML/File[@id=$fid]/@name"/>:<xsl:value-of select="$n/@lineno"/></xsl:message>
    <xsl:value-of select="/castxml2pony/CastXML/File[@id=$fid]/@name"/>:<xsl:value-of select="$n/@lineno"/>
