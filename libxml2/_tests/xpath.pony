...
class \nodoc\ iso TestXPath is UnitTest
  ...
  fun apply(h: TestHelper) =>
    ...
    try
      let xmlbad: Xml2Doc = Xml2Doc.parseFile(FileAuth(h.env.root), "Idontexist")?
    ...
    try
      ...
      let xmldoc: Xml2Doc = Xml2Doc.parseFile(FileAuth(h.env.root), "./libxml2/_tests/libxml2-2.0.gir")?
        h.assert_eq[USize]((xmldoc.xpathEval("//*", ns)            as Array[Xml2Node]).size(), 15)
        h.assert_eq[USize]((xmldoc.xpathEval("/gi:repository", ns) as Array[Xml2Node]).size(), 1)
        h.assert_eq[USize]((xmldoc.xpathEval("//gi:record", ns)    as Array[Xml2Node]).size(), 8)
        ...
        var res: Array[Xml2Node] = (xmldoc.xpathEval("//gi:namespace", ns) as Array[Xml2Node])
        h.assert_eq[USize](res.size(), 1)
        var xmln: Xml2Node = res(0)?
        h.assert_eq[String](xmln.name(), "namespace")
        h.assert_eq[USize]((xmln.xpathEval("./gi:record", ns) as Array[Xml2Node]).size(), 8)
